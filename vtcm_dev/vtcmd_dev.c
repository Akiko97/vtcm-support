...
#ifdef DEBUG
#define debug(fmt, ...)                                                        \
	printk(KERN_DEBUG "%s %s:%d: Debug: " fmt "\n", TCM_MODULE_NAME,       \
	       __FILE__, __LINE__, ##__VA_ARGS__)
#else
#define debug(fmt, ...)
#endif
#define info(fmt, ...)                                                         \
	printk(KERN_INFO "%s %s:%d: Info: " fmt "\n", TCM_MODULE_NAME,         \
	       __FILE__, __LINE__, ##__VA_ARGS__)
#define error(fmt, ...)                                                        \
	printk(KERN_ERR "%s %s:%d: Error: " fmt "\n", TCM_MODULE_NAME,         \
	       __FILE__, __LINE__, ##__VA_ARGS__)
#define alert(fmt, ...)                                                        \
	printk(KERN_ALERT "%s %s:%d: Alert: " fmt "\n", TCM_MODULE_NAME,       \
	       __FILE__, __LINE__, ##__VA_ARGS__)
...
#define TCM_TAG_RQU_AUTH1_COMMAND                                              \
	0xC200 /* An authenticated command with one authentication
                                                  handle */
#define TCM_TAG_RQU_AUTH2_COMMAND                                              \
	0xC300 /* An authenticated command with two authentication
                                                  handles */
#define TCM_TAG_RSP_COMMAND                                                    \
	0xC400 /* A response from a command with no authentication
                                                */
#define TCM_TAG_RSP_AUTH1_COMMAND                                              \
	0xC500 /* An authenticated response with one authentication
                                                  handle */
#define TCM_TAG_RSP_AUTH2_COMMAND                                              \
	0xC600 /* An authenticated response with two authentication
                                                  handles */

// vtcm special TAG
#define TCM_TAG_RQU_VTCM_COMMAND                                               \
	0xD100 /* An authenticated response with two authentication
                                                  handles */
#define TCM_TAG_RSP_VTCM_COMMAND                                               \
	0xD400 /* An authenticated response with two authentication
                                                  handles */
#define TCM_TAG_RQU_MANAGE_COMMAND                                             \
	0xE100 /* An authenticated response with two authentication
                                                  handles */
#define TCM_TAG_RSP_MANAGE_COMMAND                                             \
	0xE400 /* An authenticated response with two authentication
                                                  handles */
...
static void netlink_rcv_msg(struct sk_buff *skb)
{
	...
	if (skb->len >= nlmsg_total_size(0)) {
		...
		debug(" return_head %x %x %x %x", umsg[0], umsg[1], umsg[4],
		      umsg[5]);
		...
		case TCM_TAG_RSP_VTCM_COMMAND:
			...
			vtcm_dev->state = VTCM_STATE_RET;
			vtcm_dev->timeout = 0;
			memcpy(vtcm_dev->res_buf,
			       umsg + sizeof(*vtcm_return_head),
...
{
	struct vtcm_device *dev =
		container_of(inode->i_cdev, struct vtcm_device, cdev);
...
static ssize_t tcm_read(struct file *file, char *buf, size_t count,
			loff_t *ppos)
{
	int ret;
	uint32_t read_size = 0;

	struct vtcm_device *vtcm_dev = file->private_data;

	debug("%s(%zd %d)", __FUNCTION__, count, MINOR(vtcm_dev->devno));
	...
	if (vtcm_dev->state != VTCM_STATE_RET) {
		...
		if ((vtcm_dev->state == VTCM_STATE_SEND) ||
			...
			if (vtcm_dev->timeout != 0) {
				int outtime = jiffies_to_msecs(
					get_jiffies_64() - vtcm_dev->timeout);
...
static ssize_t tcm_write(struct file *file, const char *buf, size_t count,
			 loff_t *ppos)
{
	int ret;
...
static long tcm_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == TCMIOC_TRANSMIT) {
		...
		vtcm_dev->state = VTCM_STATE_SEND;
		vtcm_dev->timeout = get_jiffies_64();
		debug("%s(%lld, %d, %d)", __FUNCTION__, vtcm_dev->timeout,
		      count, MINOR(vtcm_dev->devno));
		wait_for_completion(&vtcm_dev->vtcm_notice);
		if (vtcm_dev->state == VTCM_STATE_RET) {
			count = ntohl(*(uint32_t *)(vtcm_dev->res_buf + 2));
			ret = copy_to_user((char *)arg, vtcm_dev->res_buf,
					   count);
			...
			if ((nlsk != NULL) &&
				...
				if (i != 0) {
					vtcm_cmd_head =
						(struct vtcm_manage_cmd_head
							 *)(vtcm_dev->cmd_buf -
							    sizeof(*vtcm_cmd_head));
					vtcm_cmd_head->tag =
						TCM_TAG_RQU_VTCM_COMMAND;
					vtcm_cmd_head->paramSize = ntohl(
						count + sizeof(*vtcm_cmd_head));
					...
					ret = tcmd_send_comm(vtcm_dev->cmd_buf,
							     count);
				}
				if (ret >= 0) {
...
*/
	...
	for (i = 0; i < VTCM_DEFAULT_NUM; i++) {
		...
		if (i == 0) {
			...
			name_offset = 4 + 1 +
				      _set_name_no(i, vtcm_device[i].name + 4);
		}
		vtcm_device[i].devno = MKDEV(major, i);
		memset(vtcm_device[i].uuid, 0, DIGEST_SIZE);
		vtcm_device[i].data_buf =
			kmalloc(VTCM_CMD_BUF_SIZE, GFP_KERNEL);
		if (vtcm_device[i].data_buf == NULL)
			return -ENOMEM;
		if (vtcm_device[i].data_buf == NULL)
			return -ENOMEM;
		vtcm_device[i].cmd_buf = vtcm_device[i].data_buf +
					 sizeof(struct vtcm_manage_cmd_head);
		vtcm_device[i].res_buf =
			vtcm_device[i].cmd_buf + VTCM_CMD_BUF_SIZE / 2;
		printk("create vtcm dev %s devno %d!\n", vtcm_device[i].name,
		       vtcm_device[i].devno);
		device_create(vtcm_class, NULL, vtcm_device[i].devno, NULL,
			      vtcm_device[i].name);
	...
	if (nlsk == NULL) {
		...
		printk("connect succeed!\n");
		vtcm_io_task = kthread_run(vtcm_io_process, vtcm_device,
					   "vtcm_io_process");
	}
...
void __exit cleanup_tcm_module(void)
{
	...
	for (i = 0; i < VTCM_DEFAULT_NUM; i++) {
		...
		device_destroy(
			vtcm_class,
			vtcm_device[i]
				.devno); // delete device node under /dev//必须先删除设备，再删除class类
	}
