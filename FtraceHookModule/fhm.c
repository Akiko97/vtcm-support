...
struct netlink_kernel_cfg cfg = { .input = rcv_msg };
		    ...
		    struct linux_binprm *bprm)
{
	...
	int i = 0;
	char **args = NULL;
	char *arg[INSERT_ARG_NUM] = { "-tpmdev", NULL, "-device",
				      "tpm-tis,tpmdev=tpm0" };
	int flag = 0;
	char uuid[VM_UUID_SIZE];
	int u_argc = argc;
	if (!strcmp(QEMU_PATH, bprm->filename)) {
		count++;
		if (count == BEFORE_PUSH_ARG) {
			...
			wait_for_completion_timeout(&done, WAIT_TIME);
			if (status == NOMSG) {
				printk("netlink receive message timeout.\n");
				ret = -ETIME;
				goto out;
			}
			...
			real_copy_strings_kernel(
				INSERT_ARG_NUM, (const char *const *)arg, bprm);
			vfree(arg[1]);
			arg[1] = NULL;
...
static int __init ftrace_hook_init(void)
{
	...
	real_copy_strings_kernel = (COPY_STRINGS_KERNEL_T)kallsyms_lookup_name(
		"copy_strings_kernel");
	memset(vtcm_name, 0, TPM_DEV_SIZE);
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION(
	"Ftrace Hook Module: A Part of Cloud Environment Transparent TCM Support Mechanism");
MODULE_VERSION("0.1");
MODULE_ALIAS("FtraceHookM");
